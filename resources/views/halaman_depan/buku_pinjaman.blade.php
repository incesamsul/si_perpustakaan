                                    ...
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            <a href="{{ URL::to('/detail_buku/' . $row->id_buku) }}" class="text-secondary">
                                                <img src="{{ asset('data/gambar_buku/' . $row->buku->gambar) }}"
                                        ...
                                        @if ($title != 'Belum diambil')
                                            <td class="align-middle">
                                                {{ $row->tgl_pinjam == '' ? 'none' : $row->tgl_pinjam }}</td>
                                            <td class="align-middle">
                                                {{ $row->tgl_kembali == '' ? 'none' : $row->tgl_kembali }}</td>
                                            <?php
                                            $jmlTerlambat = dateDiff($row->tgl_pinjam, $tanggal_hari_ini) - $row->jml_hari;
                                            ?>
                                            <td class="align-middle">{{ $jmlTerlambat < 0 ? '0' : $jmlTerlambat }} Hari
                                            </td>
                                            <td class="align-middle">
                                                {{ $jmlTerlambat < 0 ? '0' : 'Rp. ' . number_format($jmlTerlambat * 500) }}
                                            </td>
                                        @endif
                                        <td class="align-middle">
                                            ...
                                            @else
                                                @if ($title == 'Belum diambil')
                                                    <button
                                                        data-qrcode="http://127.0.0.1:8000/pustakawan/pinjamkan/{{ $row->id_user }}"
                                                        class="btn btn-lihat bg-main text-white" data-toggle="modal"
                                                        data-target="#qrcode" data-keyboard="false"
                                                        data-backdrop="static">Lihat</button>
                                                @else
                                                    <button
                                                        data-qrcode="http://127.0.0.1:8000/pustakawan/kembalikan/{{ $row->id_user }}"
                                                        class="btn btn-lihat bg-main text-white" data-toggle="modal"
                                                        data-target="#qrcode" data-keyboard="false"
                                                        data-backdrop="static">Lihat</button>
                                                @endif
